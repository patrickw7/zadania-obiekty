...
const fixData = (arr) => {
  ...
  arr.forEach((record) => {
    const [id, name, surname, email, age, cash] = record;
    newArr.push({
...
const newDatabase = fixData(users);

// 2
const allMoneyOfUsers = newDatabase.reduce((acc, user) => acc + user.cash, 0);
const avgUsersAge = newDatabase.reduce((acc, user) => acc + user.age, 0) / newDatabase.length;
const emailsOfUsers = newDatabase.map((user) => user.email);
